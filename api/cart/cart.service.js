...
class CartService {
  ...
  async buildHtml(cart, userDetails) {
    try {
      ...
      pdf.create(htmlForPdf, {format:'A4'}).toBuffer(async (err,buffer) => {
        if (err) {
          console.log(err);
        }else{
          buffers.push(buffer)
      ...
      await emailService.sendMail('הזמנה חדשה קייטרינג גבאי', htmlForEmail, userDetails.email,orderAfterSave.id,buffers);
