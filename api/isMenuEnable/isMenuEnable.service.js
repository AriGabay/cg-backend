...
class isMenuEnableService {
  setIsMenuEnable = async ({ menuType, enable }) => {
    ...
    const findIsMenuEnable = await db.IsMenuEnable.findOne({
      where: { menuType: menuType },
    });
    if (findIsMenuEnable && Object.keys(findIsMenuEnable)) {
      const updateIsMenuEnable = await db.IsMenuEnable.update(
        { enable: enable },
        { where: { menuType: menuType } }
      );
      return updateIsMenuEnable;
    }
    const isMenuEnable = await db.IsMenuEnable.create({
      enable: enable,
      menuType: menuType,
    });
    if (!Object.keys(isMenuEnable).length) {
  ...
  getAllMenuEnables = async () => {
    try {
      const findIsMenuEnables = await db.IsMenuEnable.findAll({
        where: { enable: true },
      });
